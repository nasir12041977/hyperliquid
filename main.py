...
<head>
    ...
    <style>
        ...
        .card { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.08); padding: 8px; border-radius: 10px; }
        .card h4 { margin: 0; color: #8b949e; font-size: 7px; text-transform: uppercase; letter-spacing: 0.5px; }
        .card .value { margin-top: 5px; font-size: 12px; font-weight: 700; color: #58a6ff; }
        .full-width { grid-column: span 3; background: rgba(0, 255, 163, 0.05); padding: 15px; }
        ...
        th { background: rgba(255, 255, 255, 0.05); padding: 8px; font-size: 8px; color: #8b949e; text-transform: uppercase; }
        td { padding: 8px; font-size: 10px; border-bottom: 1px solid rgba(255, 255, 255, 0.03); }
...
<div class="container">
    ...
    <div class="stats-grid">
        <div class="card full-width">
            ...
            <div class="value" style="font-size: 22px;">${{ "%.2f"|format(total_val) }}</div>
        </div>
        ...
        <div class="card"><h4>Total Margin Used</h4><div class="value">${{ margin_used }}</div></div>
        ...
        <div class="card"><h4>Live PNL</h4><div class="value {{ 'plus' if total_pnl >= 0 else 'minus' }}">${{ "%.4f"|format(total_pnl) }}</div></div>
        <div class="card"><h4>Total Positions</h4><div class="value">{{ positions|length }}</div></div>
    </div>

    <div class="pos-table">
        <div style="padding: 10px; font-size: 9px; color: #8b949e; font-weight: bold; text-align:center; border-bottom: 1px solid rgba(255, 255, 255, 0.08);">LIVE TRADE STATUS</div>
        <table>
            <thead><tr><th>COIN</th><th>SIZE</th><th>ENTRY</th><th>LEV</th><th>PNL</th><th>LIQ.PX</th></tr></thead>
            <tbody>
...
def dashboard():
    try:
        ...
        data = {
            ...
            'raw_usd': float(m_summary.get('totalRawUsd', 0)),
